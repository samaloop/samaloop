...
export default function PlacementList() {
  ...
  const data = useSWR("/api/ages/list", fetcher);
  ...
  const handleDeleteClick = async (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await supabase.from("ages").delete().eq("id", id);
          ...
          <Card.Body>
            <div className="d-md-flex justify-content-between align-items-center border-bottom pb-4 mb-4">
              ...
              <div className="text-center text-md-start">
                <Link href="/user/age/create" className="btn btn-primary">
                  Create
                </Link>
              </div>
            </div>
                  ...
                  data.data.data.map((value: any, index: number) => (
                    <tr key={"data" + index}>
                      ...
                      <td>
                        <Link
                          href={
                            "/user/age/update/" + encodeURIComponent(value.id)
                          }
                          className="text-dark"
                        >
