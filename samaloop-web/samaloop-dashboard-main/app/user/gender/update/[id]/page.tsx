...
export default function BusinessTypesCreate({
  ...
  const data = useSWR("/api/genders/detail/" + params.id, fetcher);
  ...
  useEffect(() => {
    if (mounted === false && data.data !== undefined) {
      breadcrumbStore([
        ...
        {
          href: "/user/gender/update/" + params.id,
          title: "Update",
        },
      ]);
  ...
  const onSubmit: SubmitHandler<input> = async (input) => {
    ...
    try {
      await axios.post("/api/genders/update/" + params.id, input);

      const update = await axios.get("/api/genders/list");
      mutate("/api/genders/list", update.data);

      const update2 = await axios.get("/api/genders/detail/" + params.id);
      mutate("/api/genders/detail/" + params.id, update2.data);
  ...
  return (
    <Row>
      <PageHeading heading="Update Genders" />
