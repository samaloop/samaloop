...
export default function BusinessTypesCreate({
  ...
  const data = useSWR("/api/clients/detail/" + params.id, fetcher);
  ...
  useEffect(() => {
    if (mounted === false && data.data !== undefined) {
      breadcrumbStore([
        ...
        {
          href: "/user/client/update/" + params.id,
          title: "Update",
        },
      ]);
  ...
  const onSubmit: SubmitHandler<input> = async (input) => {
    ...
    try {
      await axios.post("/api/clients/update/" + params.id, input);

      const update = await axios.get("/api/clients/list");
      mutate("/api/clients/list", update.data);

      const update2 = await axios.get("/api/clients/detail/" + params.id);
      mutate("/api/clients/detail/" + params.id, update2.data);
  ...
  return (
    <Row>
      <PageHeading heading="Update Clients" />
