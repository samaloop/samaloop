...
export default function BusinessTypesCreate({
  ...
  const data = useSWR("/api/profile/detail/" + params.id, fetcher);
  ...
  useEffect(() => {
    if (mounted === false && data.data !== undefined) {
      breadcrumbStore([
        ...
        {
          href: "/user/profile/update/" + params.id,
      ...
      if (data.data.data[0].department) {
        setDepartmentSelected({
          value: data.data.data[0].department.id,
          label: data.data.data[0].department.name,
        });
      }
      setPhone(data.data.data[0].contact.phone);
      setEmail(data.data.data[0].contact.email);
      setProfession(data.data.data[0].profession);
      setLinkedin(data.data.data[0].contact.linkedin);
      ...
      for (const value of data.data.data[0].profile_other_credentials) {
        otherCredentialsSelected.push({
          value: value.credential.id,
          label: value.credential.name,
        });
      }
      setOtherCredentialsSelected(otherCredentialsSelected);

      let specialitiesSelected = [];
      for (const value of data.data.data[0].profile_specialities) {
        specialitiesSelected.push({
          value: value.speciality.id,
          label: value.speciality.name.en,
        });
      }
      setSpecialitiesSelected(specialitiesSelected);

      if (data.data.data[0].profile_methods.length > 0) {
        setMethodsSelected({
          value: data.data.data[0].profile_methods[0].method.id,
          label: data.data.data[0].profile_methods[0].method.name.id,
        });
      }
      ...
      for (const value of data.data.data[0].profile_client_types) {
        clientTypesSelected.push({
          value: value.client_type.id,
          label: value.client_type.name.en,
        });
      }
      setClientTypesSelected(clientTypesSelected);

      let pricesSelected = [];
      for (const value of data.data.data[0].profile_prices) {
        pricesSelected.push({
          value: value.price.id,
          label: value.price.name.en,
        });
      }
      setPricesSelected(pricesSelected);

      setCroppedImage({
        url: data.data.data[0].photo,
      });

      setAwards(data.data.data[0].awards);
      setAwardsEn(data.data.data[0].awards_en);
      setDescriptionId(data.data.data[0].description.id);
      setDescriptionEn(data.data.data[0].description.en);
    }
  }, [mounted, data]);
  ...
  const formSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let photo = croppedImage.url;
      if (croppedImage.blob !== undefined) {
      ...
      await supabase.from("profile_prices").delete().eq("profile", params.id);
      ...
      await supabase.from("profile_methods").delete().eq("profile", params.id);
      if (methodsSelected !== "") {
      ...
      const update2 = await axios.get("/api/profile/detail/" + params.id);
  ...
  return (
    <Row>
      <PageHeading heading="Update Coach" />
