...
export default function BusinessTypesCreate({
  ...
  const data = useSWR("/api/hours/detail/" + params.id, fetcher);
  ...
  useEffect(() => {
    if (mounted === false && data.data !== undefined) {
      breadcrumbStore([
        ...
        {
          href: "/user/hour/update/" + params.id,
          title: "Update",
        },
      ]);
  ...
  const onSubmit: SubmitHandler<input> = async (input) => {
    ...
    try {
      await axios.post("/api/hours/update/" + params.id, input);

      const update = await axios.get("/api/hours/list");
      mutate("/api/hours/list", update.data);

      const update2 = await axios.get("/api/hours/detail/" + params.id);
      mutate("/api/hours/detail/" + params.id, update2.data);

      router.push("/user/hour");
  ...
  return (
    <Row>
      <PageHeading heading="Update Hours" />
